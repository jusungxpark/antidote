...
export function computePillarTargetRect(
  ...
  // Mobile: full-width flowing panel — match CSS `--mobile-edge` at ≤768px.
  if (vw > 0 && vw <= 768) {
    const side = Math.min(28, Math.max(22, vw * 0.06));
    return {
      top: Math.min(top, 72),
      left: side,
...
export function computeHomeCardRect(cardIndex: number): LayoutRect {
  ...
  const cardPx = vw <= 768 ? Math.max(240, Math.floor(vw - 56)) : CARD_PX;
